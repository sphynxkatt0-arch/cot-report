...
def _fetch_yahoo_price(yahoo_symbol: str) -> pd.DataFrame:
    ...
    headers = {"User-Agent": "Mozilla/5.0 COT overlay script"}
    raw: bytes
    if requests is not None:
...
def load_fred(series_id: str) -> pd.DataFrame:
    ...
    if series_id in YAHOO_PRICE_SERIES:
        ...
        # Persist to the data dir so future runs can reuse it.
        for base in [Path.cwd(), Path(__file__).resolve().parent, Path(__file__).resolve().parent.parent]:
            data_dir = base / "data"
            if data_dir.is_dir():
...
def main() -> None:
    ...
    parser.add_argument("--market", choices=["nq", "sp500", "vix", "rty", "dow", "all"], default="all")
