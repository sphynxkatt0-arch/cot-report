...
ROOT=Path(__file__).resolve().parent;SNAP=ROOT/'worldclass'/'research'/'snapshots'/'2026-08-11-release-corrected-v2';REMOTE='https://cot-v2-final-certification.vercel.app';FINAL_URL=f'{REMOTE}/FINAL_CERTIFICATION.json';SNAP_URL=f'{REMOTE}/snapshot';EXPECTED_GATE_A_SHA256='8025d66c8457789ceb0811bde3df4fc6878f74b4e62ec73d862b88dceb51a7e6';EXPECTED_B1_SHA256='11be5034064b384a9e49582e69e053fc45cf0f2e99cddfa05d617f52a6f2d2ce';EXPECTED_B2_SHA256='3abecd211ddbfd51acb858793232c73d71af0d6f5be88bf26daffdd00b92759a';EXPECTED_ACTIVE_SHA256='e205bd07ac6e35c8abc120264e013b73656e60ba6d8d5918b6ab1e1480e923d2';EXPECTED_SNAPSHOT_MANIFEST_SHA256='16d61f2937d98ab7fb21d96a73fbae58695f34a59d3fee726a056a8dbc154a8e';REQUIRED_FILES=('verification-manifest.json','SHA256SUMS.txt','BACKTEST_CORRECTNESS_REPORT.md','cot-active-edges-build-time-v2.json','cot-edge-registry-v2.json','cot-actor-event-summary.json','cot-threshold-inference-v2.json.gz','cot-actor-event-research.json.gz','directional-validation-v2.json','cot-edge-details-v2/sp500.json','cot-edge-details-v2/nq.json','cot-edge-details-v2/vix.json','cot-edge-details-v2/rty.json','cot-edge-details-v2/dow.json','cot-edge-details-v2/gold.json','cot-edge-details-v2/silver.json')
...
def load_json_bytes(data,label):
 ...
 return value
def atomic_write(path,data):path.parent.mkdir(parents=True,exist_ok=True);temp=path.with_suffix(path.suffix+'.tmp');temp.write_bytes(data);temp.replace(path)
def verify_existing():
...
def materialize(force=False):
 ...
 if int(final.get('active_compact_bytes') or 0)>180000:raise RuntimeError('certified active runtime exceeds transport budget')
