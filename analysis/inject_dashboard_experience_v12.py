...
def tone(value: Any) -> str:
    ...
    if "short" in text or "bear" in text or "defensive" in text or "invalid" in text or "override" in text:
        return "negative"
    if "wait" in text or "reduced" in text or "partial" in text or "tentative" in text:
        return "warning"
    return "neutral"
...
def playbook_card(row: dict[str, Any], fiscal_state: str, macro_coverage: Any) -> str:
    ...
    next_step = (
        "Act only after price confirmation remains aligned."
