#!/usr/bin/env python3
"""Build only the latest directional decisions and standalone report.

Canonical history and model-comparison outputs are built first by dedicated
scripts. This avoids any intermediate noncanonical history files.
"""
...
def read_validation() -> list[dict[str, Any]]:
    if not VALIDATION_OUT.exists():
        raise FileNotFoundError(
            f"Missing {VALIDATION_OUT}; run rebuild_directional_history.py before latest decisions"
        )
...
def main() -> None:
    ...
    (OUT_DIR / "cot_direction_latest.json").write_text(
        json.dumps(decisions, indent=2) + "\n",
        encoding="utf-8",
    )
    (OUT_DIR / "macro_direction_context.json").write_text(
    ...
    for decision in decisions:
        print(
            f"{decision['market_label']}: {decision['final_action']} | "
            f"{decision['structural_bias']} | release {decision['release_status']} | "
            f"exposure {decision['exposure_multiplier']:.2f}x"
        )
