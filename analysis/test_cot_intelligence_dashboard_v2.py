...
def main():
 ...
 for key,row in states.items():
  assert key==row['series'];report=row['report_date_tuesday'];assert row['release_date_friday']==release_date(report).isoformat(),(key,report,row['release_date_friday'])
  assert row.get('availability_at_utc') and row.get('release_calendar_hash')
 ...
 assert registry['horizons']==HORIZONS;assert len(registry.get('actors') or {})==59;counts=registry['research_counts'];assert counts['actor_horizon_cells']==885;assert counts['market_oi_horizon_cells']==105;assert counts['threshold_metrics']>0
 detail_series=set();detail_cells=0;oi_cells=0
 for market in MARKETS:
  detail=load(WC/'cot-edge-details'/f'{market}.json');assert detail['market']==market;assert detail['research_generation']=='release-corrected-v2';assert detail.get('actors');assert 'threshold_percentile_profiles' in detail;assert 'actor_flow_x_oi_direction' in detail
  detail_cells+=len(detail['actors']);oi_cells+=len(detail.get('market_oi') or {})
 ...
 assert detail_cells==885 and oi_cells==105,(detail_cells,oi_cells);assert len(detail_series)==59
 ...
 for _,block in (active.get('by_market') or {}).items():
  for row in block.get('active_thresholds') or []:
   total_active+=1;assert float(row['current_change_percentile'])>=float(row['selected_threshold']);assert row['direction'] in {'ADD','CUT'};assert row['production_promotion_eligible'] is False
   for metric in row.get('metrics') or []:
    assert {'conditional_return_pct','baseline_return_pct','excess_vs_baseline_pp','evidence_status','independent_n'}.issubset(metric)
    assert metric['evidence_status'] in {'GLOBAL_FDR','FAMILY_FDR','NONOVERLAP_CONFIRMED','HOLDOUT_DIRECTION_CONFIRMED','DISCOVERY_ONLY'}
