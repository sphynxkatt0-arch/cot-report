...
def main():
 ...
 assert registry['research_generation']==active['research_generation']==cross['research_generation']==prov['research_generation']=='release-corrected-v2'
 assert prov['historical_research_frozen'] is True;assert registry['automatic_promotion_allowed'] is False;assert active['automatic_promotion_allowed'] is False;assert cross['governance']['automatic_promotion_allowed'] is False
 states=current.get('actor_states') or {};assert len(states)==59,len(states)
 for key,row in states.items():
  assert key==row['series'];report=row['report_date_tuesday'];assert row['release_date_friday']==release_date(report).isoformat(),(key,report,row['release_date_friday']);assert row.get('availability_at_utc') and row.get('release_calendar_hash')
 ...
 for market in MARKETS:
  detail=load(WC/'cot-edge-details'/f'{market}.json');assert detail['market']==market;assert detail['research_generation']=='release-corrected-v2';assert detail.get('actors');assert 'threshold_percentile_profiles' in detail;assert 'actor_flow_x_oi_direction' in detail;detail_cells+=len(detail['actors']);oi_cells+=len(detail.get('market_oi') or {})
 ...
 assert active['schema_version']==5;assert active['governance']['production_model_changed'] is False;assert active['governance']['automatic_promotion_allowed'] is False;assert active['pp_definition'].startswith('percentage points');assert active['governance']['nested_threshold_policy'].startswith('one evidence-best crossed threshold')
 ...
 for _,block in (active.get('by_market') or {}).items():
  for row in block.get('active_thresholds') or []:
   ...
   for h,m in metrics.items():
    ...
    if h in FORWARD:
     assert {'baseline_return_pct','median_return_pct','positive_rate_pct','avg_drawdown_pct','worst_drawdown_pct','evidence_status','independent_n','family_fdr_q','global_fdr_q'}<=set(m)
 assert total_active==int(active.get('active_threshold_count') or 0) and total_active<=59;assert ACTIVE.stat().st_size<=180000
 assert cross['governance']['status']=='DISCOVERY_ONLY';assert cross['current_same_actor_across_markets']
 ...
 edge_model=EDGE_MODEL.read_text(encoding='utf-8');assert 'GLOBAL_FDR' in edge_model and 'NONOVERLAP_CONFIRMED' in edge_model and 'historical excess' in edge_model and 'not an absolute price prediction' in edge_model and 'cot-active-edges.json' in edge_model
 js=(WC/'cot-intelligence.js').read_text(encoding='utf-8');assert 'cot-edge-details/' in js and 'cot-cross-market.json' in js
 edge_js=EDGE_JS.read_text(encoding='utf-8');assert 'ranked, never summed' in edge_js and 'Historical backtests remain research evidence' in edge_js
 mobile_runtime=MOBILE_RUNTIME.read_text(encoding='utf-8');assert 'mobileUxReady' in mobile_runtime and 'document.head.lastElementChild' in mobile_runtime and 'calc(100vw - 24px)' in mobile_runtime and '"#wcCommandCenter"' in mobile_runtime and '"repeat(4, minmax(0, 1fr))"' in mobile_runtime
