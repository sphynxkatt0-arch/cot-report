...
def main()->None:
 ...
 try:
  ...
  provenance={'schema_version':1,'research_generation':'release-corrected-v2','snapshot_id':manifest.get('snapshot_id'),'snapshot_manifest_sha256':sha(SNAP/'verification-manifest.json'),'gate_a_archive_sha256':manifest.get('gate_a_archive_sha256'),'gate_b1_archive_sha256':manifest.get('gate_b1_archive_sha256'),'gate_b2_directional_sha256':manifest.get('gate_b2_directional_sha256'),'current_state_generated_at_utc':current.get('generated_at_utc'),'registry_sha256':sha(CANON_REG),'active_edges_sha256':sha(CANON_ACTIVE),'cross_market_sha256':sha(CANON_CROSS),'detail_hashes':{p.name:sha(p) for p in sorted(CANON_DETAILS.glob('*.json'))},'historical_research_frozen':True,'automatic_promotion_allowed':False};(WC/'cot-research-provenance.json').write_text(json.dumps(provenance,separators=(',',':'),sort_keys=True)+'\n',encoding='utf-8');install_copy_asset();print(f"Installed certified release-corrected v2 runtime · active={active.get('active_threshold_count')} · snapshot={manifest.get('snapshot_id')}")
 finally:
