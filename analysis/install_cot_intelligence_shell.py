...
def main()->None:
    for path in (HTML,JS,CSS,LIGHT_CSS,CURRENT_EDGE_MODEL,CURRENT_EDGE_JS,CURRENT_EDGE_CSS):
    ...
    print(f"Installed COT Intelligence shell assets · css={digest(CSS)} light-css={digest(LIGHT_CSS)} js={digest(JS)} current-edge={digest(CURRENT_EDGE_JS)}")
