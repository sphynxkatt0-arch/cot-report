#!/usr/bin/env python3
"""Macro-liquidity v1.2 validation layered on five-market v1.1 contracts."""
...
def validate_v12() -> None:
    ...
    if isinstance(payload, dict) and payload:
        ...
        fiscal_keys = {source.get("key") for source in payload.get("sources") or [] if isinstance(source, dict) and source.get("dataset") == "fiscaldata"}
        expected_fiscal_keys = {"treasury_operating_cash", "treasury_cash_flows", "treasury_auction_absorption"}
    ...
    if not isinstance(decisions, list) or len(decisions) != len(DIRECTIONAL_MARKETS):
        failures.append(f"v1.2 decisions must contain exactly {len(DIRECTIONAL_MARKETS)} governed markets")
    else:
        if {str(row.get('market')) for row in decisions} != set(DIRECTIONAL_MARKETS):
            failures.append("v1.2 decision market set is incomplete")
        for row in decisions:
            market = str(row.get("market") or "unknown")
            for field in ("liquidity_plumbing_guard", "liquidity_plumbing_guard_active", "liquidity_plumbing_guard_reliable"):
                if field not in row:
    ...
    for path, label in ((DASHBOARD, "dashboard"), (REPORT, "directional report")):
        ...
        for text in ("Daily Treasury Cash Flow", "Private cash effect · 5d", "Tax deposits · 5d", "Treasury Auction Demand Quality", "Average bid-to-cover delta"):
    ...
    if DASHBOARD.exists():
        ...
        for anchor in ('href="#directionalDecisionSummary"', 'href="#macroLiquidityControlRoom"', 'href="#fiscalCashPath"', 'href="#auctionAbsorption"', 'href="#directionalDecisionQuality"'):
            if anchor not in source:
                failures.append(f"dashboard navigation is missing {anchor}")
        for text in ("What to do, what must confirm", "Next action", "Plumbing guard", "Show research"):
            if text not in source:
...
def main() -> None:
    v11.main()
    validate_v12()
    print("Directional and macro-liquidity v1.2 output validation passed for all five markets.")
