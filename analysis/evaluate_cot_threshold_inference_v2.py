...
This is the statistical gate above the legacy threshold-selection engine. It
rebuilds actor events under the v2 information contract, evaluates discovery
and holdout separately, confirms effects on greedily non-overlapping episodes,
uses deterministic bootstrap uncertainty, and applies BH-FDR across both actor
families and the global searched universe.
"""
...
def values(events:list[dict[str,Any]],horizon:str)->list[float]:
    return [v for e in events if (v:=finite(event_return(e,horizon))) is not None]
...
def metric(series:str,dataset:str,market:str,actor_name:str,direction:str,threshold:int,horizon:str,events:list[dict[str,Any]])->dict[str,Any]:
    ...
    dcond=[e for e in discovery if e.get("direction")==direction and finite(e.get("magnitude_percentile")) is not None and float(e["magnitude_percentile"])>=threshold]
    hcond=[e for e in holdout if e.get("direction")==direction and finite(e.get("magnitude_percentile")) is not None and float(e["magnitude_percentile"])>=threshold]
    dvals=values(dcond,horizon);dbase=values(discovery,horizon);hvals=values(hcond,horizon);hbase=values(holdout,horizon)
    independent_cond=nonoverlap(hcond,horizon);independent_base=nonoverlap(holdout,horizon);ivals=values(independent_cond,horizon);ibase=values(independent_base,horizon)
    dedge=(mean(dvals)-mean(dbase)) if dvals and dbase else None;hedge=(mean(hvals)-mean(hbase)) if hvals and hbase else None;iedge=(mean(ivals)-mean(ibase)) if ivals and ibase else None
    same_sign=dedge is not None and hedge is not None and dedge*hedge>0;independent_sign=hedge is not None and iedge is not None and hedge*iedge>0
    boot=bootstrap_edge(ivals,ibase,SEED+abs(hash((series,direction,threshold,horizon)))%1000000)
    classification="DISCOVERY_ONLY"
    if same_sign and len(hvals)>=MIN_HOLDOUT_N:classification="HOLDOUT_DIRECTION_CONFIRMED"
