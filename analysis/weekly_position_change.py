#!/usr/bin/env python3
"""Add week-over-week positioning changes to the governed COT decision output.

This layer is descriptive only. It explains what changed between the two latest
common Legacy/TFF report dates and never changes direction, exposure, or action.
"""
...
def difference(current: Any, previous: Any, digits: int = 4) -> float | None:
    ...
    previous_value = finite(previous)
...
def category_change(
    ...
    frame = legacy if spec["source"] == "legacy" else tff
...
def weekly_signal_state(
    current_score: float | None,
    previous_score: float | None,
    *,
    threshold: float = 0.25,
    material_delta: float = 0.10,
) -> str:
    current = finite(current_score)
    previous = finite(previous_score)
...
def score_snapshot(
    ...
    snapshot = feature_snapshot(
        ...
        tff,
        report_ts,
        int(config["minimum_history_weeks"]),
    )
    ...
    return {
        **snapshot,
        ...
        "adjusted_cot_score": preserve_structural_sign(structural, tactical),
    }


def build_market_change(
    market: str,
    legacy: pd.DataFrame,
    tff: pd.DataFrame,
    config: dict[str, Any],
    ...
    if len(dates) < 2:
        return {
            ...
            "weekly_signal_change": "Change unavailable",
            "weekly_signal_material": False,
            "position_changes": [],
        }

    previous_ts, current_ts = dates[-2], dates[-1]
    current = score_snapshot(legacy, tff, current_ts, config)
    previous = score_snapshot(legacy, tff, previous_ts, config)
    changes = [
        category_change(spec, legacy, tff, current_ts, previous_ts)
        for spec in CATEGORY_SPECS
    ]
    state = weekly_signal_state(
        current["adjusted_cot_score"],
        previous["adjusted_cot_score"],
    )
    material_position_move = any(
        abs(float(item["delta_net_oi_pct"])) >= 0.5
        for item in changes
        if item.get("delta_net_oi_pct") is not None
    )
    return {
        "market": market,
        ...
        "noncommercial_percentile_change": difference(
            current["noncommercial_percentile"], previous["noncommercial_percentile"], 2
        ),
        "asset_manager_percentile_change": difference(
            current["asset_manager_percentile"], previous["asset_manager_percentile"], 2
        ),
        "other_reportable_trend13_rank_change": difference(
            current["other_reportable_trend13_rank"], previous["other_reportable_trend13_rank"]
        ),
        "nonreportable_trend13_rank_change": difference(
            current["nonreportable_trend13_rank"], previous["nonreportable_trend13_rank"]
        ),
        "noncommercial_flow4_rank_change": difference(
            current["noncommercial_flow4_rank"], previous["noncommercial_flow4_rank"]
        ),
...
def change_panel(rows: list[dict[str, Any]]) -> str:
    ...
    for row in rows:
        position_rows = "".join(
            ...
            f"<td>{fmt_delta(item.get('delta_net_oi_pct'), 2, ' pp')}</td></tr>"
            for item in row.get("position_changes") or []
        ) or "<tr><td colspan='3'>Position change unavailable</td></tr>"
        cards.append(
            f"<article class='weekly-change-card'><div class='weekly-change-kicker'>"
            f"{html.escape(str(row.get('market_label') or row.get('market')))}</div>"
            f"<h3>{html.escape(str(row.get('weekly_signal_change') or 'Change unavailable'))}</h3>"
            f"<p>Versus report {html.escape(str(row.get('previous_report_date') or 'n/a'))}; "
            f"adjusted COT score {fmt_delta(row.get('adjusted_cot_score_change'))}.</p>"
...
def main() -> None:
    ...
    for market in ("sp500", "nq"):
        legacy, tff, _prices = load_market_inputs(market)
        change = build_market_change(market, legacy, tff, config)
        changes_by_market[market] = change
        for item in change.get("position_changes") or []:
            detailed_rows.append({
                "market": market,
                "report_date": next(
                    (row.get("report_date") for row in decisions if row.get("market") == market),
                    None,
                ),
    ...
    HTML_OUT.write_text(
        inject_panel(HTML_OUT.read_text(encoding="utf-8", errors="replace"), change_panel(enriched)),
        encoding="utf-8",
    )
    for row in enriched:
        print(
            f"{row.get('market_label')}: {row.get('weekly_signal_change')} | "
            f"score delta {row.get('adjusted_cot_score_change')}"
        )
