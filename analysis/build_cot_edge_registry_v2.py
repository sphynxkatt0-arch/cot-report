...
def main()->None:
    ...
    if threshold.get("strict_release_alignment") is not True:raise RuntimeError("v2 registry requires strict release alignment")
    ...
    for key,rows in grouped.items():
        ...
        threshold_edges[key]={
            ...
            "best_classification":classification,"best_independent_n":best.get("independent_n"),"best_family_fdr_q":best.get("family_fdr_q"),"best_global_fdr_q":best.get("global_fdr_q"),
            "horizons":{str(r.get("horizon")):{k:r.get(k) for k in ("classification","holdout_n","holdout_edge_pp","independent_n","independent_edge_pp","ci95_low_pp","ci95_high_pp","block_bootstrap_p","bootstrap_se_pp","block_length_weeks","family_fdr_q","global_fdr_q")} for r in rows},
    ...
    registry={
        ...
        "sources":{"threshold_inference":{"path":str(THRESHOLD.relative_to(ROOT)),"sha256":sha(THRESHOLD)},"raw_position_oi":{"path":str(RAW.relative_to(ROOT)),"sha256":sha(RAW)}},
        ...
        "threshold_inference":"circular moving-block bootstrap + non-overlapping confirmation + family/global BH-FDR",
