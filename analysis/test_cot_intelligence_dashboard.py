...
ROOT=Path(__file__).resolve().parent; WC=ROOT/"worldclass"; CURRENT=WC/"cot-current-state.json"; REGISTRY=WC/"cot-edge-registry.json"; ACTIVE=WC/"cot-active-edges.json"; POLICY=ROOT/"config"/"cot_edge_promotion_policy.json"; HTML=ROOT/"worldclass_dashboard.html"
...
def main():
 current=load(CURRENT); registry=load(REGISTRY); active=load(ACTIVE); policy=load(POLICY); assert current["information_contract"]["lookahead_safe"] is True; assert current["production_model_changed"] is False; states=current.get("actor_states") or {}; assert len(states)==59,len(states)
