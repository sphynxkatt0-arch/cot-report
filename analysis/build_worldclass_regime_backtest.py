...
WORLDCLASS=ROOT/"worldclass";BASE=WORLDCLASS/"base.json";METALS=WORLDCLASS/"metals.json";OUT=WORLDCLASS/"regime_backtest.json"
MODEL_SPEC=cot_bt.MODEL_SPEC;MODEL_VERSION=cot_bt.MODEL_VERSION;MODEL_SPEC_HASH=cot_bt.MODEL_SPEC_HASH
THRESHOLDS=MODEL_SPEC["thresholds"];ANALOG_SPEC=MODEL_SPEC["analogs"]
BULLISH_THRESHOLD=float(THRESHOLDS["bullish"]);BEARISH_THRESHOLD=float(THRESHOLDS["bearish"])
EXTREME_UPPER=float(THRESHOLDS["extreme_upper_percentile"]);EXTREME_LOWER=float(THRESHOLDS["extreme_lower_percentile"])
MIN_SAMPLE=int(ANALOG_SPEC["minimum_regime_sample"]);DISPLAY_ANALOGS=int(ANALOG_SPEC["display_count"])
MOMENTUM_WEIGHT=float(ANALOG_SPEC["momentum_weight"]);EXTREME_COUNT_WEIGHT=float(ANALOG_SPEC["extreme_count_weight"])
MACRO_SCORE_WEIGHT=float(ANALOG_SPEC["macro_score_weight"]);TRANSMISSION_MISMATCH_PENALTY=float(ANALOG_SPEC["transmission_mismatch_penalty"])
...
def quantile(values:list[float],q:float)->float|None:
    ...
    f=pos-lo;return clean[lo]*(1-f)+clean[hi]*f
...
def macro_on_or_before(rows:list[dict[str,Any]],target:date)->dict[str,Any]|None:
    ...
    idx=bisect_right([r["date"] for r in rows],target)-1
...
def summary(records:list[dict[str,Any]],horizon:str,baseline:list[dict[str,Any]])->dict[str,Any]:
    ...
    avg=statistics.mean(returns);std=statistics.pstdev(returns) if len(returns)>=2 else 0.0;base=statistics.mean(base_returns) if base_returns else None
    return {"mean_return_pct":round(avg,4),"median_return_pct":round(statistics.median(returns),4),"hit_rate_pct":round(sum(v>0 for v in returns)/len(returns)*100,2),"q10_return_pct":round(quantile(returns,.10),4),"q25_return_pct":round(quantile(returns,.25),4),"q75_return_pct":round(quantile(returns,.75),4),"q90_return_pct":round(quantile(returns,.90),4),"worst_return_pct":round(min(returns),4),"max_drawdown_pct":round(min(drawdowns),4) if drawdowns else None,"risk_adjusted":round(avg/std,3) if std>1e-12 else None,"observations":len(returns),"baseline_return_pct":round(base,4) if base is not None else None,"excess_return_pct":round(avg-base,4) if base is not None else None,"confidence":confidence(len(returns))}
...
def build_market_dataset(market:str,dataset:str,payload:dict[str,Any],prices_payload:Any,macro_rows:list[dict[str,Any]])->dict[str,Any]|None:
    ...
    for index in range(cot_bt.MIN_LOOKBACK_WEEKS-1,len(rows)):
        ...
        if not release_meta.get("research_eligible") or not release_meta.get("actual_release_date"):
            excluded_unresolved+=1;continue
        entry=release_aligned_entry(prices,report_date)
        if entry is None:continue
        release_target=date.fromisoformat(str(release_meta["actual_release_date"]));signal_index=int(entry["index"])
    ...
    return {"market":market,"dataset":dataset,"model_version":MODEL_VERSION,"model_spec_hash":MODEL_SPEC_HASH,"methodology":{"lookahead_safe_cot":True,"macro_vintage_safe":False,"macro_evidence_status":"CALENDAR_ALIGNED_NOT_VINTAGE_SAFE","cot_release_anchor":"first close on/after canonical CFTC public availability; unresolved release weeks excluded","unresolved_release_policy":"EXCLUDE","release_calendar_aware":True,"release_calendar_hash":calendar_hash(),"macro_alignment":"last dated macro observation on or before canonical COT availability; present-day revised series may remain","cot_state_thresholds":f"bullish >={BULLISH_THRESHOLD:g}, bearish <={BEARISH_THRESHOLD:g}, otherwise neutral","macro_state_thresholds":f"bullish >={BULLISH_THRESHOLD:g}, bearish <={BEARISH_THRESHOLD:g}, otherwise neutral"},"current":current,"families":families,"regime_history":[{"report_date":r["report_date"],"release_target_date":r["release_target_date"],"signal_date":r["signal_date"],"cot_state":r["cot_state"],"cot_score_delta_4w":round(r.get("cot_score_delta_4w") or 0,2),"extreme_count":int(r.get("extreme_count") or 0),"macro_state":r["macro_state"],"transmission_state":r["transmission_state"],"combined_state":f"{r['cot_state']} COT / {r['macro_state']} macro"} for r in history[-260:]],"historical_signal_count":len(realized),"excluded_unresolved_release_rows":excluded_unresolved}
