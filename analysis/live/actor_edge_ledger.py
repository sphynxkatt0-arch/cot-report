...
This namespace is deliberately separate from the production COT/macro/combined
ledger. It can accumulate prospective research evidence without changing or
invalidating the existing forecast schema.
"""
...
class EdgeLedgerError(RuntimeError): pass
...
def release_vintage_utc(release: str|date)->datetime:
    d=release if isinstance(release,date) else parse_day(release);return datetime(d.year,d.month,d.day,21,35,tzinfo=STOCKHOLM).astimezone(UTC)
...
def validate_forecast(f:dict[str,Any])->None:
    ...
    if release!=report+timedelta(days=3):raise EdgeLedgerError("release must equal Tuesday report +3 days")
    if f.get("created_at_utc")!=iso_utc(release_vintage_utc(release)):raise EdgeLedgerError("created_at must be deterministic 21:35 Stockholm vintage")
...
def validate_ledger(root:Path)->dict[str,Any]:
    ...
    entries={}
    for p in entry_files(root):e=load(p);sid=str(e.get("signal_id") or "");
