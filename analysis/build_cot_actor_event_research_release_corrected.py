...
RESEARCH_GENERATION="release-corrected-v2";INFORMATION_CONTRACT_VERSION="cftc-public-availability-v2";ORIGINAL_BUILD=legacy.build_market_actor_events
...
def build_market_actor_events(market:str,dataset:str,payload:dict[str,Any],prices_payload:Any):
    original_index=backtest.first_price_index_on_or_after;backtest.first_price_index_on_or_after=alignment.first_price_index_on_or_after
    try:events=ORIGINAL_BUILD(market,dataset,payload,prices_payload)
    ...
    for actor_events in (events or {}).values():
        for event in actor_events:
            ...
            if prices[idx]["date"]<release_day:raise AssertionError(f"lookahead actor event {market}/{dataset}/{event.get('actor')} {report}: {prices[idx]['date']} < {release_day}")
            event.update({"release_date":release_day.isoformat(),"availability_at_utc":meta["availability_at_utc"],"availability_source_type":meta["availability_source_type"],"release_calendar_version":meta["calendar_version"],"release_calendar_hash":meta["release_calendar_hash"],"research_generation":RESEARCH_GENERATION,"information_contract_version":INFORMATION_CONTRACT_VERSION,"weekday_cumulative":{k:legacy.r4(v) for k,v in exact_path(release_day,prices,by_date,idx).items()}})
    return events

def stamp(path):
    if not path.exists():return
    p=json.loads(path.read_text(encoding="utf-8"));p["research_generation"]=RESEARCH_GENERATION;p["information_contract_version"]=INFORMATION_CONTRACT_VERSION;p["release_calendar_hash"]=calendar_hash();p["legacy_engine_preserved_for_audit"]=True
...
def main():
    original=legacy.build_market_actor_events;legacy.build_market_actor_events=build_market_actor_events
    try:legacy.main()
    finally:legacy.build_market_actor_events=original
