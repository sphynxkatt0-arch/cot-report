...
def metric_payload(actor,series,direction,threshold,horizon,inf):
 ...
 if horizon in WEEKDAYS:return {'horizon':horizon,'n':p.get('n'),'conditional_return_pct':p.get('conditional_return_pct'),'excess_vs_baseline_pp':p.get('excess_vs_baseline_pp')}
 return {'horizon':horizon,**p,'evidence_status':inf.get('classification'),'independent_n':inf.get('independent_n'),'family_fdr_q':inf.get('family_fdr_q'),'global_fdr_q':inf.get('global_fdr_q')}
def main():
 ...
 inf_by_key=defaultdict(dict)
 for metric in inference.get('metrics') or []:inf_by_key[(str(metric.get('series')),str(metric.get('direction')),int(metric.get('threshold') or 0))][str(metric.get('horizon'))]=metric
 states=current.get('actor_states') or {};crossed=defaultdict(list)
 ...
 for series,edges in crossed.items():
  state=states[series];edge=max(edges,key=lambda e:(RANK.get(str(e.get('best_classification') or 'DISCOVERY_ONLY'),0),int(e.get('threshold') or 0),int(e.get('best_independent_n') or 0)));direction=str(state.get('direction') or 'FLAT');threshold=int(edge.get('threshold') or 0);evidence=inf_by_key.get((series,direction,threshold),{});metrics=[]
 ...
 output['active_threshold_count']=output['active_edge_count'];OUT.write_text(json.dumps(output,separators=(',',':'),sort_keys=True)+'\n',encoding='utf-8')
 if OUT.stat().st_size>180000:raise RuntimeError(f'active-edge v2 runtime too large: {OUT.stat().st_size}')
