#!/usr/bin/env python3
"""Inject the latest multi-market directional decision into the macro dashboard."""
...
def build_block(rows: list[dict[str, Any]]) -> str:
    ...
    for row in rows:
        ...
        report_family = str(row.get("secondary_report") or "secondary")
        cards.append(f"""
        <article class="directional-summary-card {tone(str(row.get('final_action') or ''))}">
          ...
          <div class="directional-meta">COT {html.escape(str(row.get('report_date') or 'n/a'))} · {html.escape(report_family)} · release {html.escape(release)} · macro {fmt(row.get('macro_regime_score'), 0, '/100')}</div>
        </article>
        """)
    delayed = [row for row in rows if row.get("release_status") in {"delayed", "catch_up_delayed"}]
    notice = ""
    if delayed:
...
(function () {{
  ...
  function relabelResearchSurfaces() {{
    document.querySelectorAll('.summary-card').forEach(function (card) {{ var label = card.querySelector('.summary-label'); if (label && label.textContent.trim() === 'Regime') setText(label, 'Selected-report research regime'); }});
    document.querySelectorAll('.card-title').forEach(function (title) {{ var value = title.textContent.trim(); if (value === 'Weekly Desk Scanner') setText(title, 'Participant Research Scanner'); if (value === 'Signal Regime Panel') setText(title, 'Selected-Report Regime Research'); }});
    document.querySelectorAll('.card-meta').forEach(function (meta) {{ var value = meta.textContent.trim(); if (value.indexOf('Ranks local SP/NQ/VIX setups') === 0) setText(meta, 'Exploratory participant ranking; the headline direction is fixed by the decision panel above.'); if (value.indexOf('COT + factor percentile-score engine') === 0) setText(meta, 'Research-only selected-report score; it does not replace the headline directional model.'); }});
    var datasetLabel = document.querySelector('label[for="dataset"]'); if (datasetLabel) setText(datasetLabel, 'Research report view');
  }}
  window.addEventListener('DOMContentLoaded', function () {{ relabelResearchSurfaces(); var observer = new MutationObserver(relabelResearchSurfaces); observer.observe(document.body, {{ childList: true, subtree: true }}); window.setTimeout(relabelResearchSurfaces, 250); }});
}}());
</script>
{END}"""
...
def remove_existing(source: str) -> str:
    ...
    return source[:start] + source[end + len(END):] if start >= 0 and end >= start else source
